...
public class Methods {
    ...
    public void extras() {
        ...
        if (message.equalsIgnoreCase(getName())) {
            panic = true;
    ...
    public static void printAllPrayersWithin() {
        for (Player player : Players.getNearest()) {
            ...
            checkPlayers(getPlayerName(player));
        }
    }
    ...
    public static void printNpcs() {
        for (Npc n : Npcs.getNpcs()) {
            ...
            // System.out.println(m.getName());
            // System.out.println(getPlayerName(player));
            System.out.println(getNpcName(n));
        }
    }

    public static String getPlayerName(Player p) {
        try {
            Field accessor = org.rev317.min.api.wrappers.Character.class
                    .getDeclaredField("accessor");
            accessor.setAccessible(true);
            Object instanceObject = accessor.get(p);
            Field name = instanceObject.getClass().getDeclaredField("ao");
            name.setAccessible(true);// even though its public
            return (String) name.get(instanceObject);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static String getNpcName(Npc n) {
        try {
            Field accessor = org.rev317.min.api.wrappers.Npc.class
                    .getDeclaredField("accessor");
            accessor.setAccessible(true);
            Object instanceObject = accessor.get(n);
            Field name = instanceObject.getClass().getDeclaredField("ao");
            name.setAccessible(true);// even though its public
            return (String) name.get(instanceObject);
        } catch (Exception e) {
    ...
    static String[] staffNames = { "Isildur", "Kris tolmeth", "Slashy",
            "Bergeron", "Billy", "Mike", "Rapsey", "Donderstone2", "Maurits",
            "Daan", "Nickrock234", "Smdjagex", "Ratonhaketon", "Vizz",
            "Sir Ivy", "Vincent G", "Swaggit", "Monkoflord", "Mitcheld12",
            "Hayden", "P000p", "The Ryannn", "Patel", "Steffy", "Rick",
            "Lottery Dude", "Tom brady", "Im on meth", "Toeh5", "Smdjagex",
            "Ryan", "Vizz", "Amount", "Iefke666", "Brutaltanks", "Strdargoba",
            "Orangepike", "Mims7099", "Lkn", "Geo", "Reece", "Pizzaroll94",
            "Gio", "Jousipamppu" };
    ...
    public void teleAl() {
        ...
        if (Game.getOpenBackDialogId() == 2492) {
            Mouse.getInstance().click(265, 440, true);
            sleep(1000);
            Mouse.getInstance().click(265, 400, true);
            sleep(2000);
            return;
        }
        ...
        sleep(250);
        Mouse.getInstance().click(650, 305, true);
        sleep(750);
    ...
    public void teleDrags() {
        ...
        Mouse.getInstance().click(new Point(745, 185));// tab
        sleep(200);
        Mouse.getInstance().click(570, 360, true);
        sleep(1500);
        Mouse.getInstance().click(635, 190, true);// backpack
        Menu.sendAction(315, -1, 421, 15240);
        sleep(1000);
    ...
    public void openDeposit() {
        ...
        if (Game.getOpenInterfaceId() != 23350) {
            bank.interact(0);
            sleep(bank.getLocation().distanceTo() * 400);
            return;
        }
    }
    ...
    public void enterHouse(String name) {
        ...
        if (Game.getOpenBackDialogId() == 2492) {
            Mouse.getInstance().click(new Point(270, 415));
            sleep(2000);
    ...
    public boolean inRandom() {
        ...
        try {
            ...
            for (Npc n : Npcs.getNpcs()) {
                if (n != null && n.getLocation().distanceTo() <= 1
                        && n.getDef().getId() >= 4000
                        && !isNormal(n.getDef().getId())) {// 3597 toy mouse
                    if (n.getLocation().getY() == me.getLocation().getY()
                            || n.getLocation().getX() == me.getLocation()
                            .getX()) {
                        rNPC = n;
    ...
    public int normalNpcs[] = { 4248, 6816, 5092, 5089, 5095, 5096, 5099, 6343,
    ...
    public static String getName() {
        try {
            Class<?> c = Loader.getClient().getClass();
            Field f = c.getDeclaredField("hQ");
            f.setAccessible(true);
            return "" + f.get(Loader.getClient());
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
    ...
    public static String cleanIt() {
        try {
            Class<?> c = Loader.getClient().getClass();
            Field f = c.getDeclaredField("hP");
            f.setAccessible(true);
            return (String) f.get(Loader.getClient());
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
    ...
    public boolean banned() {
        if (getFirstLine().contains("anned") || getSecLine().contains("anned")) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isLoggedIn() {
        return true; //assume user must be logged in
//        try {
//            Class<?> c = Loader.getClient().getClass();
//            Field f = c.getDeclaredField("ao");
//            f.setAccessible(true);
//            return (boolean) f.get(Loader.getClient());
//        } catch (NoSuchFieldException e) {
//            e.printStackTrace();
...
//        return false;
    ...
    public void teleLumb() {
        ...
        if (Game.getOpenBackDialogId() == 2492) {
            Mouse.getInstance().click(260, 380, true);
            sleep(1500);
            return;
        }
    ...
    public void teleSeers() {
        ...
        Mouse.getInstance().click(new Point(745, 185));// tab
        sleep(200);
        Mouse.getInstance().click(640, 310, true);
        sleep(1000);
        Mouse.getInstance().click(635, 190, true);// backpack
